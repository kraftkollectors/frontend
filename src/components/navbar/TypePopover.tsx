...
export default function TypePopover() {
    ...
    const q = query ? typeof query == "string" ? query : (query ?? []).join(" ") : '';
                ...
                <div className="flex flex-col items-stretch">
                    {
                        links.map(link => {
                            ...
                            return <button key={link.title}
                                ...
                                className={`inline-flex text-start items-center gap-2 py-2 rounded-lg px-3 w-[150px] text-label font-semibold text-black-300 hover:bg-black-50 ${active ? 'text-black-500' : ''}`}>
                                {link.icon}
                                <span className="w-full">{link.title}</span>
                                {
                                    active &&
                                    <svg width="12" height="10" viewBox="0 0 12 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M4.0001 7.77989L1.2201 4.9999L0.273438 5.93989L4.0001 9.66656L12.0001 1.66656L11.0601 0.726562L4.0001 7.77989Z" fill="#121212" />
                                    </svg>
