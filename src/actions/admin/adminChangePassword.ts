...
export async function adminChangePassword(_: any, formData: FormData): Promise<ActionResponse> {
    ...
    // debugLog(data);
    if(data.confirmPassword !== data.password || data.oldPassword.length < 6) return({error: "Passwords do not match"})

    try {
        const req = await ServerApiRequest.patch(apis.adminChangePassword(data.adminId), data);
