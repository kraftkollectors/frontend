...
export async function markContactOrReportResolved(_res: ActionResponse, formData: FormData): Promise<ActionResponse> {
    ...
    try {
        const req = await (data.feedbackId
            ? ServerApiRequest.patch(apis.singleFeedback(data.feedbackId), { data, resolved: true })
            : ServerApiRequest.patch(apis.singleReport(data.reportId!), { data, resolved: true }));
        const res = (await req?.json()) as ApiResponse
        debugLog(res);
        if (res.statusCode == 201) {
            revalidateTag(name == 'feedback' ? tags.feedbacks : tags.reports)
            return { success: "Resolved "+name, data: res.data };
        }
        return { error: res.data ?? "Failed to update"+name, data: res };
    } catch (error) {
        debugLog(error);
