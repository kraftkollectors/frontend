...
import { TotalUserReviews } from "@/utils/types/review";


export async function fetchUserReviewsCount({ throwsError = true, isPublic = false, params }: ServerActionParams = {}): Promise<ActionApiResponse<TotalUserReviews>> {
    try {
        const accessId = isPublic ? "" : cookies().get(appCookies.accessId)?.value
        const req = await (isPublic ? ApiRequest.getJson(apis.userReviewsCount(params ?? ''), {
            next: { revalidate: 60 * 60 * 5 },
        }) : ServerApiRequest.get(apis.userReviewsCount(accessId ?? ''), {
            next: { revalidate: 60 * 60 * 5 },
        }));
        if(!req) return null;
